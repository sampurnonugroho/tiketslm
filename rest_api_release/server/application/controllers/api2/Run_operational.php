...
class Run_operational extends REST_Controller {
	...
	function index_get() {
        ...
        if ($id == '') {
									...
									FROM cashtransit_detail
										...
										LEFT JOIN client_cit ON(IF(cashtransit_detail.id_pengirim=0, cashtransit_detail.id_penerima, cashtransit_detail.id_pengirim)=client_cit.id)
											WHERE 
												...
												IFNULL(client.sektor, client_cit.sektor) NOT IN (SELECT run_number FROM runsheet_operational WHERE id_cashtransit=cashtransit.id)
						), 0) AS count
						FROM
							cashtransit
							LEFT JOIN
			...
			// $q = "SELECT * FROM cashtransit_detail 
					// LEFT JOIN client ON(cashtransit_detail.id_bank=client.id) 
					// LEFT JOIN client_cit ON(cashtransit_detail.id_pengirim=client_cit.id) 
					// WHERE cashtransit_detail.id_cashtransit=cashtransit.id AND 
					// client.sektor NOT IN (SELECT run_number FROM runsheet_operational WHERE id_cashtransit=cashtransit.id) AND
					// client_cit.sektor NOT IN (SELECT run_number FROM runsheet_operational WHERE id_cashtransit=cashtransit.id)
	...
	function get_data_post() {
		...
		$qry = "select SQL_CALC_FOUND_ROWS * from runsheet_operational WHERE id_cashtransit='".$id."' limit $offset,$rows";
		...
		foreach($query as $row){
			...
			$items[$i]['custodian_1'] = $row->custodian_1;
			$items[$i]['custodian_2'] = $row->custodian_2;
			$i++;
		}
		$result["rows"] = $items;
	...
	function suggest_data_client_post() {
		...
		// $sql = "SELECT * FROM cashtransit_detail 
				...
				// WHERE 
					...
					// client.sektor NOT IN (SELECT run_number FROM runsheet_operational WHERE id_cashtransit='$id_cashtransit') AND
				...
				// SELECT cashtransit_detail.id, client_cit.sektor as sektor_1, client.sektor as sektor_2 FROM cashtransit_detail 
					// LEFT JOIN client ON(cashtransit_detail.id_bank=client.id) 
					// LEFT JOIN client_cit ON(cashtransit_detail.id_pengirim=client_cit.id)
					// LEFT JOIN master_zone ON(client_cit.sektor=master_zone.id AND client.sektor=master_zone.id) 
				// WHERE 
					// cashtransit_detail.id_cashtransit='$id_cashtransit' AND 
					// client.sektor NOT IN (SELECT run_number FROM runsheet_operational WHERE id_cashtransit='$id_cashtransit') AND
					// client_cit.sektor NOT IN (SELECT run_number FROM runsheet_operational WHERE id_cashtransit='$id_cashtransit')
					// GROUP BY client_cit.sektor
