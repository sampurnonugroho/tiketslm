...
class Run_logistic extends REST_Controller {
									...
									FROM cashtransit_detail
										...
										LEFT JOIN client_cit ON(IF(cashtransit_detail.id_pengirim=0, cashtransit_detail.id_penerima, cashtransit_detail.id_pengirim)=client_cit.id)
											WHERE 
												...
												IFNULL(client.sektor, client_cit.sektor) NOT IN (SELECT run_number FROM runsheet_logistic WHERE id_cashtransit=cashtransit.id)
						), 0) AS count
						FROM
							cashtransit
							LEFT JOIN
	...
	function get_data_post() {
		...
		$qry = "select SQL_CALC_FOUND_ROWS * from runsheet_logistic WHERE id_cashtransit='".$id."' limit $offset,$rows";
	...
	function suggest_data_client_post() {
		...
		// $sql = "SELECT * FROM cashtransit_detail 
				...
				// WHERE 
					...
					// client.sektor NOT IN (SELECT run_number FROM runsheet_logistic WHERE id_cashtransit='$id_cashtransit') AND
