<?php $__env->startSection('content'); ?>
	...
	<script type="text/javascript">
				...
				$('#dg').datagrid({
					...
					onExpandRow: function(index,row){
						...
						ddv.panel({
							...
							href:'<?=base_url()?>ticket_slm/show_form?index='+index,
							onLoad:function(){
		...
		function saveItem(index){
			...
			console.log(row);
			var url = row.isNewRecord ? '<?=base_url()?>ticket_slm/save_data' : '<?=base_url()?>ticket_slm/update_data?id='+row.id;
			console.log(url);
			...
			if (row){
				$.messager.confirm('Confirm','Are you sure you want to remove this user?',function(r){
					if (r){
						...
						console.log(row);
						$.post('<?=base_url()?>ticket_slm/delete_data',{id:row.id},function(){
							$('#dg').datagrid('deleteRow',index);
						});
					}
				});
	...
	<section class="grid_14">
			...
			<form class="block-content form" id="complex_form" method="post" action="#">
				<h1>SLM Troubleshoot Tickets</h1>
				...
				<div class="columns">
					<table id="dg" class="easyui-datagrid" title="List Of Troubleshoot Tickets - Second Level Maintenance" style="width:100%;height:470px"
						data-options="rownumbers:true,singleSelect:true,url:'<?=base_url()?>ticket_slm/get_data',method:'get'" toolbar="#toolbar" pagination="true">
						<thead>
							<tr>
