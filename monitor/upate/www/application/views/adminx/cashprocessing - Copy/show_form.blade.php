	...
	<div>
		<table class="dv-table" style="float:left;border:1px solid #ccc;padding:5px;margin-top:5px;width:400px">
			...
			<tr>
				<td>Petty Cash</td>
				<td>:</td>
				<td><input name="petty_cash" id="nama_denom" class="easyui-validatebox" style="width:100%" required="true"></input></td>
			</tr>
		</table>
		<table class="dv-table" style="float:left;border:1px solid #ccc;padding:5px;margin-top:5px;width:400px">
			<tr>
...
<script type="text/javascript">
	...
	{
		console.log(src);
		var page    = '<?=base_url()?>cashprocessing/suggest';
		console.log(page);
		if(src.length>=2){
	...
	jq341(document).ready(function()
	{
		jq341('.run_sheet_number').select2({
			...
			ajax: {
				dataType: 'json',
				url: '<?php echo base_url().'cashprocessing/suggest'?>',
				delay: 250,
				type: "POST",
