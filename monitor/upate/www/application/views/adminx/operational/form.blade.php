			...
			$('#dg').datagrid({
				...
				onExpandRow: function(index,row){
					...
					ddv.panel({
						...
						href:'<?=base_url()?>operational/show_form?index='+index+'&row='+JSON.stringify(row)+'&id='+<?=$id?>,
						onLoad:function(){
			...
			if (row){
				$.messager.confirm('Confirm','Are you sure you want to remove this user?',function(r){
					if (r){
						...
						console.log(row);
						$.post('<?=base_url()?>operational/delete_data',{id:row.id},function(data){
							console.log(data);
							$('#dg').datagrid('deleteRow',index);
						});
					}
	...
	<section class="grid_12">
			<div class="block-border"><form class="block-content form" id="complex_form" method="post" action="#">
				...
				<table id="dg" class="easyui-datagrid" title="Cash Transit <?=$branch?>" style="width:100%;height:550px"
					data-options="rownumbers:true,singleSelect:true,url:'<?=base_url()?>operational/get_data/<?=$id?>',method:'get'" toolbar="#toolbar" pagination="true">
					<thead>
						<tr>
							<th data-options="field:'run_number',width:200">Run Sheet Area</th>
