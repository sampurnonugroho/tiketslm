		...
		<div class="block-border">
					...
					<div class="container_16">
						...
						<?php if($session->userdata['level']=="LEVEL1") { ?>
							<div class="float-right">
								<!--<button type="button" onClick="window.location.href='<?=base_url()?>handover_out/add'">Add Data</button>-->
			...
			<div class="widget_wrap preview_table">
				<div class="widget_top">
					...
					<h6>Outgoing <?=ucwords(explode("_", $active_menu)[0])?> Data</h6>
				</div>
							...
							<tr>
								...
								<?php if($session->userdata['level']=="LEVEL1") { ?>
									<td>
										...
										<span><a class="button" onClick="window.location.href='<?php echo base_url();?>handover_out/add_ho/<?php echo $row->id;?>'" href="#" id="print_bast_done" title="Detail">Detail</a></span>
										<span><a class="action-icons c-delete" onClick="openDelete('<?php echo $row->id;?>', '<?php echo base_url();?>handover_out/delete')" href="#" title="delete">Delete</a></span>
									</td>
								<?php } ?>
							</tr>
							<?php 
				...
				'<form class="form">'+
					'<fieldset>'+
						...
						'<p>'+
							...
							'<input type="text" id="ho_to" placeholder="" value="" class="full-width" required>'+
						'</p>'+
						'<p>'+
			...
			$.modal({
				...
				buttons: {
					'Yes': function(win) { 
						...
						$.ajax({
							url: '<?=base_url()?>handover_out/save_data',
							dataType: 'html',
							type: 'POST',
							data: {
							...
							success: function(data) {
								// console.table(data);
								window.location.href = '<?=base_url()?>handover_out/add_ho/'+data;
							}
						});
					},
