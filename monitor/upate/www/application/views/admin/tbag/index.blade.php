...
@section('content')
							...
							<tr>
								...
								<td>
									...
									<?php if($session->userdata['level']=="LEVEL1") { ?>
										<span>
											<a class="action-icons c-delete" onClick="openDelete('<?php echo $row->id;?>', '<?php echo base_url();?>tbag/delete')" href="#" title="delete">Delete</a>
										</span>
									<?php } ?>
								</td>
							</tr>
	...
	<script>
		$(document).on('click', '#detail_preview', function(){ 
			...
			setTimeout(function() {
				var websel = "<?=base_url()?>pdf/qrcode_tbag/"+id;
		...
		function openModalTambah() {
			...
			var content = ''+
				'<form class="form">'+
					'<fieldset>'+
						'<p>'+
							'<label>Dari</label>'+
							'<input type="text" id="dari_tbag" class="full-width">'+
						'</p>'+
						'<p>'+
							'<label>Hingga</label>'+
							'<input type="text" id="hingga_tbag" class="full-width">'+
						'</p>'+
					'</fieldset>'+
				'</form>'+
			'';
			...
			$.confirm({
				...
				buttons: {
					Submit: function () {
						var dari = $("#dari_tbag").val();
						var hingga = $("#hingga_tbag").val();
						...
						$.confirm({
							title: 'Success!',
							content: 'url:<?=base_url()?>tbag/save/'+that_data,
