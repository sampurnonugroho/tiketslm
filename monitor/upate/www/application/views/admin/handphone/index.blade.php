					...
					<div class="container_16">
						...
						<?php if($session->userdata['level']=="LEVEL1" OR $session->userdata['nama_dept']=="GA & LOGISTIC") { ?>
							<div class="float-right">
								<button type="button" onClick="window.location.href='<?=base_url()?>handphone/add'">Add Data</button>
							</div>
						<?php } ?>
						<center>
				...
				<div class="widget_content">
					<table class="display data_tbl">
						...
						<tbody>
							<?php 
								$no = 0;
								if(!empty($datahandphone))
								foreach($datahandphone as $row): 
								$no++;
							?>
							<tr>
								...
								<?php if($session->userdata['level']=="LEVEL1") { ?>
									<td>
										<span><a class="action-icons c-edit" onClick="window.location.href='<?php echo base_url();?>handphone/edit/<?php echo $row->id;?>'" href="#" title="Edit">Edit</a></span>
										<span><a class="action-icons c-delete" onClick="openDelete('<?php echo $row->id;?>', '<?php echo base_url();?>handphone/delete')" href="#" title="delete">Delete</a></span>
									</td>
								<?php } ?>
							</tr>
							<?php 
