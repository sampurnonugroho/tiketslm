...
class Ticket_slm extends CI_Controller {
    ...
    public function add_ticket_slm() {
		...
		return view('admin/ticket_slm/add', $this->data);
	}
	...
	function  update_data() {
		...
		$query = "SELECT * FROM slm_trouble_ticket WHERE id_ticket='".$_REQUEST['id_ticket']."'";
		$num_rows = json_decode($this->curl->simple_get(rest_api().'/select/query', array('query'=>$query), array(CURLOPT_BUFFERSIZE => 10)));
		
		if(count($num_rows)==0) {
			$query = "INSERT INTO slm_trouble_ticket (`id_ticket`, `id_bank`, `problem_type`, `time`, `status`, `data_solve`)
						SELECT id_ticket, id_bank, problem_type, time, status, data_solve FROM flm_trouble_ticket
			...
			$query = "UPDATE slm_trouble_ticket SET teknisi_2='".$_REQUEST['teknisi_2']."' WHERE id_ticket='".$_REQUEST['id_ticket']."'";
			json_decode($this->curl->simple_get(rest_api().'/select/query', array('query'=>$query), array(CURLOPT_BUFFERSIZE => 10)));
		} else {		
			$query = "UPDATE slm_trouble_ticket SET teknisi_2='".$_REQUEST['teknisi_2']."' WHERE id_ticket='".$_REQUEST['id_ticket']."'";
			json_decode($this->curl->simple_get(rest_api().'/select/query', array('query'=>$query), array(CURLOPT_BUFFERSIZE => 10)));
		...
		// $this->db->insert('slm_trouble_ticket', $data);

		// $this->db->trans_complete();
		
		$row = json_decode($this->curl->simple_get(rest_api().'/select/query', array('query'=>"SELECT * FROM slm_trouble_ticket LEFT JOIN client ON(slm_trouble_ticket.id_bank=client.id) WHERE slm_trouble_ticket.id_ticket='".$_REQUEST['id_ticket']."'"), array(CURLOPT_BUFFERSIZE => 10)));;
		$branch = json_decode($this->curl->simple_get(rest_api().'/select/query', array('query'=>"SELECT * FROM master_branch WHERE id='$row->cabang'"), array(CURLOPT_BUFFERSIZE => 10)));
	...
	function status_slm() {
		...
		$this->data['statusflm'] = $this->db->query("SELECT *, slm_trouble_ticket.status as status_ticket FROM slm_trouble_ticket LEFT JOIN client ON(slm_trouble_ticket.id_bank=client.id)")->result();
