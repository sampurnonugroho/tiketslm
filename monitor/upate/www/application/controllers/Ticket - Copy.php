...
class Ticket extends CI_Controller {
    ...
    public function add_ticket() {
		...
		return view('admin/ticket/add', $this->data);
	}
	...
	function get_data() {
		...
		$query = $this->db->query("SELECT count(*) as cnt FROM flm_trouble_ticket LEFT JOIN client ON(flm_trouble_ticket.id_bank=client.id)");
        $row = $query->row_array();
		$result["total"] = $row['cnt'];
		
		$query = $this->db->query("SELECT * FROM flm_trouble_ticket LEFT JOIN client ON(flm_trouble_ticket.id_bank=client.id)  WHERE flm_trouble_ticket.status!='SLM' limit $offset,$rows");
	...
	function select_problem_flm() {
		...
		WHERE kategori.nama_kategori='FIRST LEVEL MAINTENANCE' AND nama_sub_kategori LIKE '$search'";	
		// echo $sql;
		$result = json_decode($this->curl->simple_get(rest_api().'/select/query_all', array('query'=>$query), array(CURLOPT_BUFFERSIZE => 10)));
		...
		if (count($result) > 0) {
			$key=0;
			foreach ($result as $row) {
				$list[$key]['id'] = $row->id_sub_kategori;
