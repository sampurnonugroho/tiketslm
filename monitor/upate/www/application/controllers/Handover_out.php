...
class Handover_out extends CI_Controller {
    var $data = array();
	...
	public function index() {
        $this->data['active_menu'] = "handover_out";
		...
		$sql = "UPDATE `outgoing_client_ho` SET `status`='onprogress' WHERE wsid NOT IN (SELECT wsid FROM client)";
		$this->curl->simple_get(rest_api().'/select/query2', array('query'=>$sql), array(CURLOPT_BUFFERSIZE => 10));
		
		// $sql = "SELECT *, outgoing_client_ho.alamat as lokasi FROM outgoing_client_ho LEFT JOIN karyawan ON(outgoing_client_ho.custodian=karyawan.nik) ORDER BY outgoing_client_ho.id DESC";
		$sql = "SELECT * FROM outgoing_detail_ho ORDER BY date ASC";
		$result = json_decode($this->curl->simple_get(rest_api().'/select/query_all', array('query'=>$sql), array(CURLOPT_BUFFERSIZE => 10)));
		
		// print_r($result);
		
		$sql_run = "SELECT MAX(run) as run_number FROM outgoing_detail_ho WHERE date='".date('Y-m-d')."'";
	...
	public function save_data() {
		...
		$cnt_detail = json_decode($this->curl->simple_get(rest_api().'/select/query', array('query'=>
			"SELECT count(*) as cnt FROM outgoing_detail_ho WHERE run='$run_number' AND date='".date('Y-m-d')."'"
		), array(CURLOPT_BUFFERSIZE => 10)))->cnt;
		
		if($cnt_detail==0) {
			$id = $this->curl->simple_get(rest_api().'/select/query2', array('query'=>
				"
				INSERT INTO `outgoing_detail_ho`
					(`run`, `date`, `bank`, `custodian`, `police_number`, `guard`, `ho_to`) VALUES 
					('$run_number','".date('Y-m-d')."','$bank','$custodian','$vehicle','$guard','$ho_to')
				"
			), array(CURLOPT_BUFFERSIZE => 10));
		} else {
			$id = json_decode($this->curl->simple_get(rest_api().'/select/query', array('query'=>
				"SELECT MAX(id) as id FROM outgoing_detail_ho"
			), array(CURLOPT_BUFFERSIZE => 10)))->id;
		}
		
		if($id!=="") {
			$cnt_outgoing_client_ho = json_decode($this->curl->simple_get(rest_api().'/select/query', array('query'=>
				"SELECT count(*) as cnt FROM outgoing_client_ho WHERE id_detail='$id'"
			), array(CURLOPT_BUFFERSIZE => 10)))->cnt;
			
			if($cnt_outgoing_client_ho<$jumlah_lokasi) {
				$cnt = $jumlah_lokasi - $cnt_outgoing_client_ho;
				
				for($i=0; $i<$cnt; $i++) {
					$this->curl->simple_get(rest_api().'/select/query2', array('query'=>
						"INSERT INTO `outgoing_client_ho` (`id`, `id_detail`, `bank`, `wsid`, `alamat`, `detail`, `custodian`, `ctr`, `reject`, `status`, `data_handover`, `updates`, `updated`) VALUES (NULL, '$id', NULL, NULL, NULL, NULL, NULL, NULL, NULL, 'onprogress', NULL, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);"
					), array(CURLOPT_BUFFERSIZE => 10));
				}
			}
		}
	...
	public function add_ho() {
		$this->data['active_menu'] = "handover_out";
		$this->data['url'] = "handover_out/save";
		$this->data['flag'] = "add";
        ...
        return view('admin/handover_out/form', $this->data);
	}
	...
	public function get_data() {
		...
		$query = json_decode($this->curl->simple_get(rest_api().'/select/query_all', array('query'=>
			"
				SELECT * FROM outgoing_client_ho WHERE id_detail='$id' LIMIT $offset, $rows
			"
		), array(CURLOPT_BUFFERSIZE => 10)));
	...
	public function get_data_client() {
		$sql = "SELECT * FROM client LEFT JOIN client_ho ON(client_ho.wsid=client.wsid) WHERE client.status='1'";
		...
		if (count($result) > 0) {
			...
			foreach ($result as $row) {
				...
				$list[$key]['wsid'] = $row->wsid.' '.$row->lokasi; 
				$list[$key]['lokasi'] = $row->lokasi; 
				$list[$key]['type'] = $detail->type; 
				$list[$key]['paket'] = $detail->paket; 
				$list[$key]['tgl_min_dari'] = $detail->tgl_min_dari; 
				$list[$key]['tgl_min_hingga'] = $detail->tgl_min_hingga; 
				$list[$key]['tgl_max_dari'] = $detail->tgl_max_dari; 
				$list[$key]['tgl_max_hingga'] = $detail->tgl_max_hingga; 
				$list[$key]['ctr'] = $detail->ctr; 
				$list[$key]['reject'] = $detail->reject; 
				$list[$key]['limit_min'] = $detail->limit_min; 
				$list[$key]['limit_max'] = $detail->limit_max; 
				$list[$key]['denom'] = $detail->denom; 
				$list[$key]['interval_isi'] = $detail->interval_isi; 
				$list[$key]['sifat'] = $detail->sifat; 
				$list[$key]['tgl_efektif'] = $detail->tgl_efektif; 
				$key++;
			}
	...
	public function show_form() {
		...
		$row = json_decode($this->input->get('row'));
		$this->data['flag'] = "show_form";
		$this->data['id'] = $id;
		$this->data['index'] = $index;
		$id_detail = $row->id_detail;
		$bank = json_decode($this->curl->simple_get(rest_api().'/select/query', array('query'=>
			"SELECT bank FROM outgoing_detail_ho WHERE id='$id_detail'"
		), array(CURLOPT_BUFFERSIZE => 10)))->bank;
		$this->data['bank'] = $bank;
		
		return view('admin/handover_out/show_form', $this->data);
	}
	...
	public function update_data() {
		...
		$status = json_decode($this->curl->simple_get(rest_api().'/select/query', array('query'=>
			"SELECT status FROM outgoing_client_ho WHERE id='$id'"
		), array(CURLOPT_BUFFERSIZE => 10)))->status;
		...
		$table = "outgoing_client_ho";
		$this->curl->simple_get(rest_api().'/select/update', array('table'=>$table, 'data'=>$data), array(CURLOPT_BUFFERSIZE => 10));
		
		
		$cnt_detail = json_decode($this->curl->simple_get(rest_api().'/select/query', array('query'=>
			"SELECT count(*) as cnt FROM outgoing_client_ho WHERE id_detail='$id_detail' AND wsid IS NULL"
		), array(CURLOPT_BUFFERSIZE => 10)))->cnt;
		
		if($cnt_detail==0) {
			// NOTIFICATION
			$query = "SELECT * FROM outgoing_detail_ho LEFT JOIN user ON(user.username=outgoing_detail_ho.custodian) WHERE outgoing_detail_ho.id='$id_detail'";
		...
		$row = json_decode($this->curl->simple_get(rest_api().'/select/query', array('query'=>
			"SELECT * FROM outgoing_client_ho WHERE id='$id'"
		), array(CURLOPT_BUFFERSIZE => 10)));
	...
	public function add() {
		...
		return view('admin/handover_out/form2', $this->data);
	}
	...
	public function edit($id) {
		$this->data['active_menu'] = "handover_out";
		$this->data['url'] = "handover_out/update";
		$this->data['flag'] = "edit";
		
		$sql = "SELECT * FROM outgoing_client_ho WHERE id = '$id'";
	...
	public function delete() {
		...
		// $query = "SELECT wsid FROM outgoing_client_ho WHERE id='".$data['id']."'";
	...
	public function suggest_custodi() {
		...
		// $sql = "SELECT * FROM user LEFT JOIN karyawan ON(user.username=karyawan.nik) WHERE user.level='CUSTODI' AND user.username NOT IN (SELECT custodian FROM outgoing_client_ho WHERE status='onprogress')";
