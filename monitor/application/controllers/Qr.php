...
class Qr extends CI_Controller {
	...
	public function print_receipt() {
		...
		$this->pdf->setPaper('A4', 'potrait');
		$this->pdf->filename = "qrcode-receipt-".$wsid.".pdf";
	...
	public function gen() {
		...
		if (strpos($from, "PS") !== false) {
			$fr = intval(str_replace("PS", "", $from));
			$to = intval(str_replace("PS", "", $to));
			
			$spintf = '%0'.strlen(str_replace("PS", "", $from)).'d';
			
			for($i=$fr; $i<=$to; $i++) {
				$qrCode = new QrCode("PS".sprintf('%0'.count($fr).'d', $i));
				$qrCode->setSize(300);
				$qrCode->writeFile(realpath(__DIR__ . '/../../upload/tes').'/PS'.sprintf($spintf, $i).'.png');
			}
			if(!isset($html)) {
				for($i=$fr; $i<=$to; $i++) {
					$template_content .= '
						<div class="">
							<div class="">
								...
								<center>
									<img style="margin-top: 18px" src="'.realpath(__DIR__ . '/../../upload/tes').'/PS'.sprintf($spintf, $i).'.png" width="90" height="90"></img>
								</center>
								<center style="margin-top: 1px; font-size: 12px; font-weight: bold">
									<span>PS'.sprintf($spintf, $i).'</span>
								</center>
							</div>
						</div>
					';
			...
			';
		} else if (strpos($from, "a") !== false) {
			$fr = intval(str_replace("a", "", $from));
			$to = intval(str_replace("a", "", $to));
			
			$spintf = '%0'.strlen(str_replace("a", "", $from)).'d';
			
			for($i=$fr; $i<=$to; $i++) {
				$qrCode = new QrCode("a".sprintf('%0'.count($fr).'d', $i));
				$qrCode->setSize(300);
				$qrCode->writeFile(realpath(__DIR__ . '/../../upload/tes').'/a'.sprintf($spintf, $i).'.png');
			}
			if(!isset($html)) {
				for($i=$fr; $i<=$to; $i++) {
					$template_content .= '
						<div class="">
							<div class="">
								...
								<center>
									<img style="margin-top: 18px" src="'.realpath(__DIR__ . '/../../upload/tes').'/a'.sprintf($spintf, $i).'.png" width="90" height="90"></img>
								</center>
								<center style="margin-top: 1px; font-size: 12px; font-weight: bold">
									<span>a'.sprintf($spintf, $i).'</span>
								</center>
							</div>
						</div>
					';
