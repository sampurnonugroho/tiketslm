...
class Runsheet extends CI_Controller {
				...
				*, 
				...
				IFNULL((
					...
					WHERE 
						...
						#AND client.sektor NOT IN (SELECT run_number FROM runsheet_security WHERE id_cashtransit=cashtransit.id) 
					GROUP BY cashtransit_detail.id_cashtransit), 0) as count 
				FROM cashtransit 
				LEFT JOIN master_branch ON(cashtransit.branch=master_branch.id) 
				LEFT JOIN (SELECT id_cashtransit, police_number, security_1, security_2 FROM runsheet_security) as runsheet_security ON(cashtransit.id=runsheet_security.id_cashtransit)
		";
	...
	public function get_data2() {
		...
		$query = "
				SELECT 
					*
					FROM
						(SELECT id_cashtransit, police_number, security_1, security_2 FROM runsheet_security) as runsheet_security
					LEFT JOIN cashtransit ON(cashtransit.id=runsheet_security.id_cashtransit)
					LEFT JOIN vehicle ON(runsheet_security.police_number=vehicle.police_number) 
					WHERE cashtransit.date='$date'";
	...
	public function detail_runsheet() {
		...
		$run_number = json_decode($this->curl->simple_get(rest_api().'/select/query', array('query'=>"SELECT run_number FROM cashtransit WHERE id='".$id_ct."'"), array(CURLOPT_BUFFERSIZE => 10)))->run_number;
