			...
			<div class="block-border"><form class="block-content form" id="complex_form" method="post" action="#">
				...
				<table class="table sortableXxx " cellspacing="0" width="100%" style="display: none">
					...
					<tbody>
						<?php 
							$no = 0;
							if(!empty($data_operational))
							foreach($data_operational as $row): 
							$no++;
						?>
						<tr>
							...
							<td style="text-align: center">
								<button type="button" class='button green' onClick="window.location.href='<?php echo base_url();?>operational/edit/<?php echo $row->date;?>'" title='Edit'><span class='smaller'>View Detail Data</span></button>
							</td>
						</tr>
						<?php 
							endforeach; 
	...
	<script>
		...
		var tables = jq3412('#example').DataTable({
			...
			"columnDefs": [
				...
				{
					"render": function ( data, type, row ) {
						return '<button type="button" class="button green" style="font-size: 12px" onClick="window.location.href=\'<?=base_url()?>operational/edit/'+row['action_date']+'\'" title=""><span class="">View Detail Data</span></button>';
					},
					"className": "text-center",
					"targets": 3,
					"searchable": false
