<?php $__env->startSection('content'); ?>
		...
		function saveItem(index){
			...
			$('#dg').datagrid('getRowDetail',index).find('form').form('submit',{
				...
				success: function(data){
					console.log(data);
					jq341.notify("Data Cash Replenish, Berhasil Disimpan", "success");
					data = eval('('+data+')');
				...
				$.messager.confirm('Confirm','Are you sure you want to remove this data?',function(r){
					if (r){
						...
						$.post('<?=base_url()?>cashreplenish/delete_data',{id:row.id},function(data){
							console.log(data);
							jq341.notify("Data Cash Replenish, Berhasil Dihapus", "warn");
							$('#dg').datagrid('deleteRow',index);
						});
					}
				});
	...
	<div id="control-bar" class="grey-bg clearfix"><div class="container_12">
		...
		<div class="float-left">
			<button type="button" onclick="window.location.href='javascript:history.go(-1)'"><img src="<?=base_url()?>constellation/assets/images/icons/fugue/navigation-180.png" width="16" height="16"> Back</button>
		</div>
	</div></div>
	<!-- End control bar -->
