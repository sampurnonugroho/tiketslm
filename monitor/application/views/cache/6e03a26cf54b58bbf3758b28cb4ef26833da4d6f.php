<?php $__env->startSection('content'); ?>
	...
	<link rel="stylesheet" type="text/css" href="<?=base_url()?>assets/constellation/assets/equipment/icon.css">
			...
			$('#dg').datagrid({
				...
				onExpandRow: function(index,row){
					...
					ddv.panel({
						...
						href:'<?=base_url()?>cashtransit/show_form?index='+index+'&row='+JSON.stringify(row)+'&id='+<?=$id?>,
						onLoad:function(){
		...
		function saveItem(index){
			...
			$('#dg').datagrid('getRowDetail',index).find('form').form('submit',{
				...
				success: function(data){
					console.log(data);
					jq341.notify("Data Cash In Transit, Berhasil Disimpan", "success");
					data = eval('('+data+')');
				...
				$.messager.confirm('Confirm','Are you sure you want to remove this data?',function(r){
					if (r){
						...
						$.post('<?=base_url()?>cashtransit/delete_data',{id:row.id},function(){
							jq341.notify("Data Cash In Transit, Berhasil Dihapus", "warn");
							$('#dg').datagrid('deleteRow',index);
						});
					}
				});
	...
	<div id="control-bar" class="grey-bg clearfix"><div class="container_12">
		...
		<div class="float-left">
			<button type="button" onclick="window.location.href='javascript:history.go(-1)'"><img src="<?=base_url()?>assets/constellation/assets/images/icons/fugue/navigation-180.png" width="16" height="16"> Back</button>
		</div>
	</div></div>
	<!-- End control bar -->
