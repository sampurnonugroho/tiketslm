			...
			$('#dg').datagrid({
				...
				onExpandRow: function(index,row){
					...
					ddv.panel({
						...
						href:'<?=base_url()?>security/show_form?index='+index+'&date=<?=$date?>',
						onLoad:function(){
		...
		function saveItem(index){
			...
			$('#dg').datagrid('getRowDetail',index).find('form').form('submit',{
				...
				success: function(data){
					console.log(data);
					jq341.notify("Data Security, Berhasil Disimpan", "success");
					data = eval('('+data+')');
				...
				$.messager.confirm('Confirm','Are you sure you want to remove this user?',function(r){
					if (r){
						...
						$.post('<?=base_url()?>security/delete_data',{id:row.id},function(){
							jq341.notify("Data Security, Berhasil Dihapus", "warn");
							$('#dg').datagrid('deleteRow',index);
						});
					}
				});
			...
			<div class="block-border">
				<form class="block-content form" id="complex_form" method="post" action="#">
					...
					<table id="dg" class="easyui-datagrid" title="Cash Transit <?=$branch?>" style="width:100%;height:550px"
						data-options="rownumbers:true,singleSelect:true,url:'<?=base_url()?>security/get_data/<?=$date?>',method:'get'" toolbar="#toolbar" pagination="true">
						<thead>
							<tr>
