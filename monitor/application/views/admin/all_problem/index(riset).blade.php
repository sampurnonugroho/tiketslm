							...
							<tbody id="upandcoming">
								<?php foreach ($demo as $d) { ?>
									<tr>
										...
										<td>(<?php echo date('H:i:s', strtotime($d['time']));?>) : <span id="demo<?php echo $d['nama'];?>"></span></td>       
									</tr>
								<?php }?>
							</tbody>
						</table>
						<script>
							var countdowns = [
								<?php foreach ($demo as $d) { ?>
									{
										id: "<?php echo $d['nama']; ?>",
										date: new Date("<?php echo date('M j, Y H:i:s', strtotime($d['time']));?>").getTime()
									},
								<?php } ?>
							];
							...
							var timer = setInterval(function() {
								...
								while(index >= 0) {
									...
									var distance = now - countdown.date;
