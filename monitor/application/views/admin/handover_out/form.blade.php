...
@section('content')
	...
	<link href="<?=base_url()?>constellation/assets/equipment/select2/select2.min.css" rel="stylesheet" />
	<script src="<?=base_url()?>constellation/assets/equipment/select2/select2.min.js"></script>
	...
	<script type="text/javascript">
			...
			$('#dg').datagrid({
				...
				onExpandRow: function(index,row){
					...
					ddv.panel({
						...
						href:'<?=base_url()?>handover_out/show_form?index='+index+'&row='+JSON.stringify(row)+'&id='+<?=$id?>,
						onLoad:function(){
		...
		function saveItem(index){
			...
			console.log(row);
			var url = row.isNewRecord ? '<?=base_url()?>handover_out/save_data' : '<?=base_url()?>handover_out/update_data?id='+row.id;
			console.log(url);
			...
			if (row){
				$.messager.confirm('Confirm','Are you sure you want to remove this user?',function(r){
					if (r){
						...
						console.log(row);
						$.post('<?=base_url()?>handover_out/delete_data',{id:row.id},function(){
							$('#dg').datagrid('deleteRow',index);
						});
					}
				});
	...
	<?=base_url()?>handover_out/get_data/<?=$id?>
		<div class="float-left">
			<button type="button" onclick="window.location.href='<?=base_url()?>handover_out'"><img src="<?=base_url()?>constellation/assets/images/icons/fugue/navigation-180.png" width="16" height="16"> Back</button>
		</div>
	</div></div>
	<!-- End control bar -->
		...
		<section class="grid_12">
			<table id="dg" class="easyui-datagrid" title="HANDOVER DETAIL" style="width:100%;height:550px"
				data-options="
					rownumbers:true,
					singleSelect:true,
					url:'<?=base_url()?>handover_out/get_data/<?=$id?>',
					method:'get',
