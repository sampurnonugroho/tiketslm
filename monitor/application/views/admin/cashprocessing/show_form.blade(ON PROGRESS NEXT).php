				...
				jq341(document).scannerDetection({
					...
					onComplete: function(barcode, qty) {
						...
						if(barcode.indexOf("A") != -1){
							...
							$.post('<?php echo base_url().'cashprocessing/check_seal'?>', { value: barcode, type: "<?=$r['act']?>" })
								.done(function( data ) {
									console.log(data);
									if(data==-1) {
							...
							onComplete: function(barcode, qty) {
								...
								if(barcode.indexOf(".") != -1){
									...
									// $.post('<?php echo base_url().'cashprocessing/check_big_seal'?>', { value: barcode, id_bank: "<?=$row->id_bank?>", id: "<?=$row->id?>", ctr: "<?=$row->ctr?>", act: "<?=$row->act?>" })
										...
										// }
									// );
									var cur_ctr = parseInt("<?=$row->ctr?>");
									var bag_ctr = barcode.substring(4, 5);
									if(cur_ctr>bag_ctr) {
										jq341.notify("Kapasistas bag tidak sesuai!", "error");
									} else {
										$.post('<?php echo base_url().'cashprocessing/check_bag'?>', { value: barcode, type: "<?=$r['act']?>" })
											.done(function( data ) {
												console.log(data);
												if(data==-1) {
								...
								if(barcode.indexOf("a") != -1){
									if($("#cart_1_seal_atm_<?=$index?>").val()=="") {
										if($("#cart_1_seal_atm_<?=$index?>").val()!==barcode && 
											...
											$.post('<?php echo base_url().'cashprocessing/check_seal_atmcrm'?>', { value: barcode, type: "<?=$r['act']?>" })
												.done(function( data ) {
													jq341.notify(data, "error");
													// if(data==-1) {
														// jq341.notify("Seal "+barcode+", tidak dikenal!", "error");
													// } else {
														// if(data==0) {
															// jq341.notify("Seal "+barcode+", sudah terpakai!", "error");
														// } else if(data==2) {
															// jq341.notify("TYPE BUKAN <?=$r['act']?>", "error");
														// } else {
															// jq341.notify("Seal "+barcode+", siap digunakan!", "success");
